...
```
```{r }
auto.arima(mydlts1,max.p=5,max.q=5,stationary=F,ic=c("bic"),trace=TRUE,allowdrift=F)
```
```
...
```
```{r }
m11 <- Arima(mydlts1, order = c(0,1,1), method = 'ML')
coeftest(m11)
plot(m11$residuals)
...
```{r}
m41=Arima(mydlts1,order=c(0,1,1),method='ML',include.drift=T)
...
```
```
#x(t)=x(t-1)+a(t)+0.2513a(a(t-1)) -- The t in this equation is suffix, unable to format in this file
