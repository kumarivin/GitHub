function [net, errTrn, errTst, errVal, ep, accTrn] = ...
	  bpstvalmv1 (network, trainset, testset, valset, targets, ...
  ...
  while (ep < epochs) && (ep < minValTargetEpochs) && (accuracy < 1)
      ...
      for i = 1:numInsts
          ...
          %% a_j := sum_k wkj xk[e]
          hidOuts1 = sigmoid(inputs(i,:)*We1);
          hidOuts2 = sigmoid([1 hidOuts1]*We2);
          %printsizem('hidOuts',hidOuts);

          %% (1x3)*(3x1)->(1x1): (1 x numOuts)
          %% a_i := sum_j wji*oj
          outOuts = sigmoid([1 hidOuts2]*We3);
      ...
      hidOuts1 = sigmoid(inputs*We1);    % 113x5 * 5x20 -> 113x20
      hidOuts2 = sigmoid([x0 hidOuts1]*We2);
      outOuts = sigmoid([x0 hidOuts2]*We3); % (instsXhids) * (hidsXouts)
      ...
      if ~isempty(testTargets)
          hidOutsTst1 = sigmoid([ones(size(testTargets,1),1) testset]*We1);
          hidOutsTst2 = sigmoid([ones(size(testTargets,1),1) hidOutsTst1]*We2);
          outOutsTst = sigmoid([ones(size(testTargets,1),1) hidOutsTst2]*We3);
          if debug1 > 1 
      ...
      if ~isempty(valTargets)
          hidOutsVal1 = sigmoid([ones(size(valTargets,1),1) valset]*We1);
          hidOutsVal2 = sigmoid([ones(size(valTargets,1),1) hidOutsVal1]*We2);
          outOutsVal = sigmoid([ones(size(valTargets,1),1) hidOutsVal2]*We3);
          if debug1 > 1 
