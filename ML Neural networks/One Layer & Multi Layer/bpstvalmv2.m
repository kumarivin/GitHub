function [net, errTrn, errTst, errVal, ep, accTrn] = ...
	  bpstvalmv2 (network, trainset, testset, valset, targets, ...
  ...
  while (ep < epochs) && (ep < minValTargetEpochs) && (accuracy < 1)
      ...
      for i = 1:numInsts
          ...
          %% a_j := sum_k wkj xk[e]
          hidOuts1 = sigmoidv1(inputs(i,:)*We1);
          hidOuts2 = sigmoidv1([1 hidOuts1]*We2);
          %printsizem('hidOuts',hidOuts);

          %% (1x3)*(3x1)->(1x1): (1 x numOuts)
          %% a_i := sum_j wji*oj
          outOuts = sigmoidv1([1 hidOuts2]*We3);
      ...
      hidOuts1 = sigmoidv1(inputs*We1);    % 113x5 * 5x20 -> 113x20
      hidOuts2 = sigmoidv1([x0 hidOuts1]*We2);
      outOuts = sigmoidv1([x0 hidOuts2]*We3); % (instsXhids) * (hidsXouts)
      ...
      if ~isempty(testTargets)
          hidOutsTst1 = sigmoidv1([ones(size(testTargets,1),1) testset]*We1);
          hidOutsTst2 = sigmoidv1([ones(size(testTargets,1),1) hidOutsTst1]*We2);
          outOutsTst = sigmoidv1([ones(size(testTargets,1),1) hidOutsTst2]*We3);
          if debug1 > 1 
      ...
      if ~isempty(valTargets)
          hidOutsVal1 = sigmoidv1([ones(size(valTargets,1),1) valset]*We1);
          hidOutsVal2 = sigmoidv1([ones(size(valTargets,1),1) hidOutsVal1]*We2);
          outOutsVal = sigmoidv1([ones(size(valTargets,1),1) hidOutsVal2]*We3);
          if debug1 > 1 
